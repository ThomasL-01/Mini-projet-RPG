...
print('\nBienvenue dans le jeu de rôle,\nVous pourrez choisir une classe et afronter un adversaire aléatoire.\nBon jeu !')
time.sleep(2)

#class définissant les spé du personnage
class Perso:
    def __init__(self,nom, type,dégats, ptsvie, classe, vitesse = 0, esquive = 0, critiques = 0):
        ...
        self.speed = vitesse #Vitesse d'attaque
        ...
        self.esquive = esquive #proba d'esquive de l'attqque
...
classes = {'assassin': {'dégats':20, 'Vie':700, 'vitesse': 25, 'esquive':30, 'critiques':50},
        'guerrier': {'dégats':20, 'Vie':1250, 'vitesse': 20, 'esquive':10, 'critiques':20},
        'mort-vivant': {'dégats':35, 'Vie':175, 'vitesse': 15, 'esquive':1, 'critiques':75},
        }
...
while not a_choisi:  #permet le choix de la classe au joueur
    ...
    if joueur_classe == '!help':
        print(' \n classe assassin (inflige des dégats élevés lors de la 1ère attaque):\n dégats: 20, Vie: 700, vitesse: 25, esquive: 30%, chance de critiques: 50%\n',
            '\n classe guerrier: dégats: 20, Vie: 1250, vitesse: 20, esquive: 10%, chance de critiques: 20% \n',
            '\n classe mort-vivant (a plusieurs vies avec des stats qui diminuent a chaque mort):\n dégats: 35, Vie: 175, vitesse: 15, esquive: 1%, chance de critiques: 75%, réanimations: 5 \n',
...
def tour_joueur(): #permet de controler le tour du joueur
    ...
    while not a_joué:
        ...
        if action == '1':
            ...
            esquive_ennemis = randint(0,100) # si < la valeur d'esquive alors esquive la prochaine attaque
...
def combat(): #pour le combat
    bot.current_speed += bot.speed
    joueur.current_speed += joueur.speed
    if bot.current_speed >= 100: #vérifie si tour du bot
        tour_bot()
        bot.current_speed -= 100
    elif joueur.current_speed >= 100: # idem  tour joueur
        tour_joueur()
        joueur.current_speed -= 100
        
en_vie = True
while en_vie: #le jeu qui tourne en entier pour les combats et les checks de mort
    if not joueur.estVivant():
        en_vie = False
        if bot.classe == 'mort-vivant':
            print('Tu as ',0,' PV et ton adversaire en a', bot.pv ,'avec',bot.reanimations,'réanimations restantes\n')
        else:
            print('Tu as',0,'PV restants et ton adversaire en a',bot.pv,'\n')
        print(bot.nom,'a gagné !\n')
    elif not bot.estVivant():
        en_vie = False
        if joueur.classe == 'mort-vivant':
            print('Tu as ',joueur.pv,'PV avec',joueur.reanimations,'réanimations restantes et ton adversaire a',0 ,'PV\n')
        else:
            print('Tu as',joueur.pv,'PV restants et ton adversaire en a',bot.pv,'\n')
